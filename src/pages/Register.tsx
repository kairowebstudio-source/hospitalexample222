    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${authBg})` }}>
      ...
      <Card className="w-full max-w-sm relative z-10 shadow-2xl border-0">
        ...
        <CardHeader className="text-center pb-2 pt-5 px-6">
          ...
          <CardTitle className="text-xl">{emailSent ? 'Check Your Email' : 'Patient Registration'}</CardTitle>
          <CardDescription className="text-xs">
            {emailSent ? `We sent a confirmation link to ${email}` : 'Create your LifeCare account'}
          </CardDescription>
        </CardHeader>
        <CardContent className="px-6 pb-5">
          {emailSent ? (
            <div className="text-center space-y-3">
              <p className="text-sm text-muted-foreground">Please click the link in your email to verify your account, then sign in.</p>
              <Link to="/login">
                <Button className="w-full h-9">Go to Login</Button>
              </Link>
            </div>
          ) : (
            <>
